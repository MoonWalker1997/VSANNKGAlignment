...
if __name__ == "__main__":
    ...
    np.save("./Saves/test_3_consistency.npy", consistency)
    np.save("./Saves/test_3_similarity.npy", similarity)
    np.save("./Saves/test_3_boolean_loss.npy", boolean_loss)
    ...
    for i, each in enumerate(consistency.mean(0)):
        plt.plot(range(epoch_VSA), each, label="kb=" + str(g_k_biases[i]))
    plt.legend()
    plt.show()
