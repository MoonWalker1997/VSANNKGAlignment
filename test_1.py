...
g_k_proportion = np.array(list(range(10, 41, 5))) / 100
...
if __name__ == "__main__":
    ...
    for _ in range(5):
        c, s, bl = [], [], []
        for r_proportion in r_proportions:  # test on the second aspect
            tc = []
            ts = []
            tbl = []
            keyword = ["_H", "_R", "_T"]
    ...
    np.save("./Saves/test_1_consistency.npy", consistency)
    np.save("./Saves/test_1_similarity.npy", similarity)
    np.save("./Saves/test_1_boolean_loss.npy", boolean_loss)
    ...
    for i, each in enumerate(consistency.mean(0)):
        plt.plot(range(epoch_VSA), each, label="%r=" + str(r_proportions[i]))
    plt.legend()
    plt.show()
