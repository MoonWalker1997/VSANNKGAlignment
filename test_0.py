...
r_proportion = np.array(list(range(10, 41, 5))) / 100
...
if __name__ == "__main__":
    ...
    for _ in range(5):  # run 5 times for each experiment and take the average
        ...
        for num_g_entities in g_num_e:  # test on the first aspect
            ...
            for ep in range(1, epoch_VSA + 1):
                ...
                # test specific task performance
                correct = 0
                with torch.no_grad():
    ...
    for i, each in enumerate(consistency.mean(0)):
        plt.plot(range(epoch_VSA), each, label="#e=" + str(g_num_e[i]))
    plt.legend()
    plt.show()
