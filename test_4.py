...
if __name__ == "__main__":
    ...
    for _ in range(5):
        ...
        for l_num_proportion in l_num_proportions:  # test on the fifth aspect
            ...
            num_l_entities = int(num_g_entities * l_num_proportion)
            l_entities = ["C_" + str(i) for i in range(num_l_entities)]
            num_l_relations = int(num_g_relations * l_num_proportion)
            l_relations = ["R_" + str(i) for i in range(num_l_relations)]
            structure_size = [num_l_relations, num_l_entities, num_l_entities]
    ...
    for i, each in enumerate(consistency.mean(0)):
        plt.plot(range(epoch_VSA), each, label="kb=" + str(l_num_proportions[i]))
    plt.legend()
    plt.show()
